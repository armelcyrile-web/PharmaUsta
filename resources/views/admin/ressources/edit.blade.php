...
<div class="container-fluid">
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white">
            <h5 class="mb-0">Modifier la ressource</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.ressources.update', $ressource) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="titre" id="titre" class="form-control @error('titre') is-invalid @enderror" value="{{ old('titre', $ressource->titre) }}" required>
                        @error('titre')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="type_ressource_id" id="type_ressource_id" class="form-select @error('type_ressource_id') is-invalid @enderror" required>
                            @foreach($typesRessources as $type)
                                <option value="{{ $type->id }}" {{ old('type_ressource_id', $ressource->type_ressource_id) == $type->id ? 'selected' : '' }}>{{ $type->nom }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="annee_academique_id" id="annee_academique_id" class="form-select @error('annee_academique_id') is-invalid @enderror" required>
                            @foreach($anneesAcademiques as $annee)
                                <option value="{{ $annee->id }}" {{ old('annee_academique_id', $ressource->annee_academique_id) == $annee->id ? 'selected' : '' }}>{{ $annee->libelle }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="niveau_id" id="niveau_id" class="form-select @error('niveau_id') is-invalid @enderror" required>
                            @foreach($niveaux as $niveau)
                                <option value="{{ $niveau->id }}" {{ old('niveau_id', $ressource->niveau_id) == $niveau->id ? 'selected' : '' }}>{{ $niveau->nom }}</option>
                            @endforeach
                        </select>
                        @error('niveau_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="ue_id" id="ue_id" class="form-select @error('ue_id') is-invalid @enderror" required>
                            @foreach($ues as $ue)
                                <option value="{{ $ue->id }}" {{ old('ue_id', $ressource->ue_id) == $ue->id ? 'selected' : '' }}>{{ $ue->nom }}</option>
                            @endforeach
                        </select>
                        @error('ue_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="ecue_id" id="ecue_id" class="form-select @error('ecue_id') is-invalid @enderror">
                            ...
                            @foreach($ecues as $ecue)
                                <option value="{{ $ecue->id }}" {{ old('ecue_id', $ressource->ecue_id) == $ecue->id ? 'selected' : '' }}>{{ $ecue->nom }}</option>
                            @endforeach
                        </select>
                        @error('ecue_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" id="description" rows="3" class="form-control @error('description') is-invalid @enderror">{{ old('description', $ressource->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <p class="text-muted">{{ $ressource->fichier }}</p>
                </div>

                <div class="mb-3">
                    <label for="fichier" class="form-label">Remplacer le fichier PDF (optionnel)</label>
                    <input type="file" name="fichier" id="fichier" class="form-control @error('fichier') is-invalid @enderror" accept=".pdf">
                    @error('fichier')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
